...
@Component({
  ...
  templateUrl: './shippings.page.html',
  styleUrls: ['./shippings.page.scss'],
})
export class ShippingsPage implements OnInit {
  ...
  search(){
    ...
    if(this.query == ""){
      ...
      this.http.post(this.url+"/shippings/search", {
        search: this.query,
        page: this.page
      }, {
        headers
  ...
  fetch(page = 1){
    ...
    if(this.query == ""){
      ...
      this.http.get(this.url+"/shippings/fetch/"+page, {
        headers
      }).subscribe((res:any) =>{
